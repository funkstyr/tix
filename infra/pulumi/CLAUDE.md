...
| `SyntheticCronJob`   | `components/synthetic-cronjob.ts`               | **Always-on (dev AND prod)** k8s CronJob (every ~2min) running `pnpm -F @tix/synthetic probe` — drives the live reserve→order→charge saga via @tix/saga-client against each service directly (per-service `*_BASE_URL`, not the gateway: the flat saga-client can't resolve the gateway's nested, auth-less oRPC router), emits `synthetic_journey_*` metrics, backs the `synthetic-journey-failure` alert + synthetics board. concurrencyPolicy Forbid, one attempt per tick. |
| `DeployAnnotation`   | `components/observability/deploy-annotation.ts` | One-shot Job (curl → Grafana annotations API) that writes a `deploy`-tagged annotation per `pulumi up` when `gitSha` is set (Job name embeds the SHA → fires once per deploy; `skipAwait` so it can't block a rollout). Dashboards overlay it as deploy markers (ADR-0010).                                                                                                                                                                                                    |
...
| `GarageBuckets`     | `garage-buckets.ts`     | one-shot Job (`curl` → Garage admin API)                                | Creates the `tempo`/`loki`/`pyroscope` buckets + imports the S3 key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `PyroscopeBackend`  | `pyroscope-backend.ts`  | Deployment + Service `pyroscope` (4040)                                 | Continuous profiling (4th pillar); S3 profile blocks in Garage (like Loki). Services push via `@pyroscope/nodejs` (env-gated in `@tix/service-runtime`); `service_name`/`service_version` tags correlate profiles↔traces. `blocks_retention_period`/`limits.retention_period` = `tracesRetention`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
metrics→`prometheus:9090/api/v1/otlp/v1/metrics`. (Each app `dependsOn`s
`observability.collector` as a _head-start_ edge — pulumi applies the collector
StatefulSet before the apps — but that orders creation, not pod readiness, so it
doesn't fully close the cold-boot race. The real guard is the OTLP **readiness
gate** in `@tix/observability` (`awaitCollector`, wired into `otlpLayer`): it
blocks the exporter's first flush until the collector is reachable, because a
not-yet-ready collector does **not** degrade gracefully — the `@effect/opentelemetry`
exporter self-disables for 60s after one failed export and drops every log in that
window, so a service that only logs at startup would otherwise vanish from Loki.)
Exposed via the
...
**Continuous profiling (4th pillar, ADR-0009 addendum).** `PyroscopeBackend` adds
profiles alongside metrics/traces/logs; the six Node services push pprof via
`@pyroscope/nodejs`, env-gated in `@tix/service-runtime` by `PROFILING_ENABLED` +
`PYROSCOPE_SERVER_ADDRESS` (both set on every Node service's `env` in `index.ts`).
...
cluster (with an ingress port-map), build + load the service images (incl. the
synthetic probe), install
...
#    `synthetic` is a CronJob (no Deployment / rollout wait) but still needs its
#    image loaded for the always-on probe.
for svc in auth tickets orders payments expiration gateway web synthetic; do
...
| `gitSha`                     | string | —                                       | Git SHA of the current deploy. When set, a `DeployAnnotation` marks it on dashboards and it's used as `SERVICE_VERSION` for profiles. From `config.get` or `GIT_SHA` env.                                                             |
| `syntheticImage`             | string | `tix-synthetic:dev`                     | Image tag for the synthetic buyer-journey probe CronJob.                                                                                                                                                                              |
| `syntheticSellerEmail`       | string | `synthetic-seller@tix.test`             | Standing synthetic seller account email the probe signs in as.                                                                                                                                                                        |
| `syntheticBuyerEmail`        | string | `synthetic-buyer@tix.test`              | Standing synthetic buyer account email the probe signs in as.                                                                                                                                                                         |
| `syntheticSellerPassword`    | secret | — (`synthetic-seller-dev` dev fallback) | Standing synthetic seller account password.                                                                                                                                                                                           |
| `syntheticBuyerPassword`     | secret | — (`synthetic-buyer-dev` dev fallback)  | Standing synthetic buyer account password.                                                                                                                                                                                            |
| `syntheticPaymentMethodId`   | string | `pm_card_visa`                          | Stripe test-mode PaymentMethod id the probe charges with (test mode only).                                                                                                                                                            |
...
## Frontend RUM (ADR-0013, ADR-0012 Tier 4)

The only tier with app code, across `apps/web` + `apps/gateway` + `@tix/observability`.

- **Collector:** a `faro` receiver on `:8090` (`otel-collector.ts`); its traces feed `traces/metrics` (spanmetrics RED) + `traces/store` (tail-sampling → Tempo), its logs feed `logs` → Loki. No new image — the contrib distro already ships `faroreceiver`.
- **Gateway:** `faro-proxy.ts` + a `POST /otel/v1/faro` route — a transparent pass-through (no span) with a body-size cap, behind the existing CORS origin check (now also allowing `traceparent`/`tracestate` headers for the stitch). Upstream from `FARO_COLLECTOR_URL` (default `http://otel-collector:8090/`).
- **SPA:** `@tix/observability/web-rum` (`initWebRum`) wires Faro + otel-web; `main.tsx` inits it when `VITE_RUM_URL` is set and feeds the normalized route pattern as the view label. The browser→backend stitch rides `traceparent` on the real RPC fetches; the gateway already parents its span from that header.
- **Board:** `web-rum` (Platform folder) — browser RED (spanmetrics, `service_name=tix-web`) + JS errors/Web Vitals (Loki).
- **Residual risk (TODO(prod)):** the RUM proxy is an unauthenticated public ingest path bounded only by the body-size cap; CORS stops browsers, not `curl`. No token/rate-limit yet (ADR-0013).
- **Verify:** unit/rendered-manifest as above; the full single-trace-id stitch + live board are a **manual dev verify**.
