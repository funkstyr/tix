...
| `SyntheticCronJob`   | `components/synthetic-cronjob.ts`               | **Always-on (dev AND prod)** k8s CronJob (every ~2min) running `pnpm -F @tix/synthetic probe` — drives the live reserve→order→charge saga via @tix/saga-client against the gateway, emits `synthetic_journey_*` metrics, backs the `synthetic-journey-failure` alert + synthetics board. concurrencyPolicy Forbid, one attempt per tick.                                                                                                               |
