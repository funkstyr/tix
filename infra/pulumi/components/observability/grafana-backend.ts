...
// Where Grafana reads provider configs and dashboard JSON. The dashboard provider's
// `path` points back at this same directory, so the provider yaml and the board JSON
// ride one ConfigMap (Grafana loads only `.json` files as dashboards, ignoring the yaml).
          ...
          template: {
            ...
            spec: {
              ...
              volumes: [
                ...
                { name: "dashboards", configMap: { name: this.dashboards.metadata.name } },
              ],
            },
          },
        },
...
// Grafana dashboard-provider config (apiVersion 1). A single file provider scans
// DASHBOARDS_PATH for `*.json` boards and files them under the Grafana `Domain` folder.
// `allowUiUpdates` lets users tweak a board live, but the change is ephemeral (no PVC) —
// surviving an edit means exporting it back to a TS definition in the repo (ADR-0010).
function renderDashboardProvider(): string {
