...
export type TempoBackendArgs = {
  ...
  storage: string;
  // How long the compactor keeps trace blocks in Garage before deleting them
  // (Go duration, e.g. `360h`). Without it Tempo keeps blocks forever and the S3
  // bucket grows unbounded. Parameterized dev-small / prod-larger.
  ...
  constructor(name: string, args: TempoBackendArgs, opts?: pulumi.ComponentResourceOptions) {
    ...
    this.config = new k8s.core.v1.ConfigMap(
      `${name}-config`,
      {
        ...
        data: {
          "tempo.yaml": renderTempoConfig(args.s3Endpoint, args.bucket, args.blockRetention),
        },
      },
      childOpts,
    );
...
function renderTempoConfig(s3Endpoint: string, bucket: string, blockRetention: string): string {
