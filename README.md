...
> In this template, we use `@tix` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@tix` to something like `@my-company` or `@project-name`.
...
The generator sets up the `package.json`, `tsconfig.json` and a `index.ts`, as well as configures all the necessary configurations for tooling around your package such as formatting, linting and type-checking. When the package is created, you're ready to go build out the package.
