...
## Tier 2 landed (2026-06)

The three signal-path-hardening items shipped as PR 2.

- **Tail-based sampling, RED kept unsampled.** `renderCollectorConfig` grew a trailing `samplingPercent` param and the single `traces` pipeline split in two sharing the one OTLP receiver: `traces/metrics` (`processors: [batch]`, `exporters: [spanmetrics, servicegraph]`) feeds the span-derived RED/service-graph connectors from the **full unsampled** stream, while `traces/store` (`processors: [batch, tail_sampling]`, `exporters: [otlp/tempo]`) applies the policy only on the way to Tempo. The `tail_sampling` processor keeps all errors (`status_code: [ERROR]`), all slow traces (`latency` ≥ 500ms), and probabilistically samples the rest at `samplingPercent`, with a 10s `decision_wait`. The rate is threaded `index.ts` → `ObservabilityStack` → `OtelCollector` → `renderCollectorConfig` and defaults to **100 in dev, 10 in prod** (`stack === "prod" ? 10 : 100`, overridable via `tix:traceSamplingPercent`) — matching the existing `loadgenEnabled`/`alertingEnabled` gating. No image change: the collector was already the contrib distro where `tail_sampling` ships.
- **Database query spans.** `@tix/observability/db-span` exports a `dbSpan(operation, table, effect)` combinator — `effect.pipe(Effect.withSpan(\`db.${operation}\`, { attributes }))`carrying`db.system=postgresql`/`db.operation`/`db.sql.table`(raw OTel DB semantic-convention keys; the`SpanAttr`vocabulary has no`db.\*`entries), a transparent sibling of`withTimeout`/`withResilience`. It is applied at the saga-critical transaction call sites across all four DB services — orders (`create_order` + four consumer dedupe spans), tickets (`reserve_ticket` + the release consumer), payments (`record_payment` + the order-projection consumer's two handlers), expiration (`schedule_expiry`) — composed `...request/consumer span... > dbSpan > withTimeout > tryPromise(db tx)` so the db span is a child of the request span and wraps the timed work. Aggregate writes outside the saga path (orders cancel, ticket create/update) were left unspanned per the plan's enumerated scope.
- **Exemplar drill-to-trace.** A new `_exemplar.ts` panel factory (`exemplarLatencyPanel`) queries the **span-derived** `duration_milliseconds_bucket{service_name=…}` (p95 + p50 via `histogram_quantile`) with `.exemplar(true)` on each target — the only series carrying the trace ID — kept deliberately separate from the hand-rolled `red-row.ts` RED panels that can't. It is wired into the saga-funnel board (`orders`) and the edge-auth board (`gateway` + `auth`, beside their RED rows), so the click-a-spike → open-the-slow-trace jump ADR-0010 wired in the datasource finally has a panel to fire from.
